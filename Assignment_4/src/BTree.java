...
public class BTree<T extends Comparable<T>> {
    ...
    //adds value to the subtree beginning with start
    public void insert(Node<T> start, T value) {
        Node<T> node = start;
        ...
        while (node.childrenSize != 0) {
            ...
            if (value.compareTo(lesser) <= 0)
                ...
                // Greater
            else if (value.compareTo(greater) > 0)
                nextNode = node.getChild(last + 1);
        ...
        //if the leaf is full split it and then find the new leaf where to insert value
        if (node.numberOfKeys() == maxKeySize) {
            T mid = node.getKey(node.keysSize / 2);
            split(node);
            node = node.parent;
            //if value is bigger then mid move to his right child
            if (value.compareTo(mid) > 0)
                node = node.getChild(node.indexOf(mid) + 1);
            //if value is smaller then mid move to his left child
            else
                node = node.getChild(node.indexOf(mid));
        }
        node.addKey(value);
    ...
    public T delete(Node<T> start, T value) {
        Node<T> node = start;
        while (node.numberOfChildren() != 0) {

            //first check if a merge / shift is required
            ...
            if (value.compareTo(lesser) < 0)
                ...
                for (int i = 0; i < numberOfKeys; i++) {
                    ...
                    //if value is in an internal node switch it with its predecessor and delete the predecessor
                    if (currentValue.compareTo(value) == 0) {
                        node = node.getChild(i);
                        Node<T> predecessor = getGreatestNode(node);
                        node.parent.setKey(i, predecessor.getKey(predecessor.keysSize - 1));
                        return delete(node, predecessor.getKey(predecessor.keysSize - 1));
                    }
                    int next = i + 1;
                    if (next <= last) {
    ...
    public boolean insert2pass(T value) {
        if (root == null) {
            ...
            //find the node where value needs to be inserted
            Node<T> node = getNode(value);
            //if no split is needed simply add value to the node
            if (node.numberOfKeys() < maxKeySize) {
                node.addKey(value);
                size++;
            } else {
                //go up the tree as long as you can't split
                while (node.parent != null && node.numberOfKeys() == maxKeySize)
                    node = node.parent;
                insert(node, value);
            }
        }
     ...
     * If value isn't in the tree get the node were it's supposed to be
     *
