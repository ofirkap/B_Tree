...
public class CuckooHashing {
    ...
    private Stack<BacktrackElement> undo; //Stack of all the inserts that can be undone
    /**
    ...
    public CuckooHashing(HashMethods hf, int size) {
        ...
        undo = new Stack<BacktrackElement>();
        makeEmpty();
        hashFunctions = hf;
    ...
    private boolean insertHelper1(String x) {
        boolean isFirst = true; //Determines if it's the original insert or a kicked out element
        while (true) {
            int pos = -1;
            int kick_pos = -1;
            ...
            for (int count = 0; count <= MAXTRIES; count++) {
                for (int i = 0; i < numHashFunctions; i++) {
                    ...
                    if (array[pos] == null) {
                        array[pos] = x;
                        currentSize++;
                        undo.push(new BacktrackElement(x, pos, isFirst));
                        return true;
                    }
                ...
                array[kick_pos] = x;
                undo.push(new BacktrackElement(x, kick_pos, isFirst));
                x = tmp;
                isFirst = false;
            }
            //insertion got into a cycle use overflow list
            this.stash.add(x);
            undo.push(new BacktrackElement(x, -1, isFirst));
            return true;
        }
    }
	...
	public void undo() {
        if(!undo.empty()){
            BacktrackElement toUndo = undo.pop();
            if (toUndo.getIndex() != -1) //check if the item was added to the stash or the array
                array[toUndo.getIndex()] = null;
            else
                stash.remove(toUndo.getValue());
            //Remove the last item from the stack and place it in its previous position
            //Loop until the original insert value is found
            while (!toUndo.isOriginal()){
                BacktrackElement temp = undo.pop();
                array[temp.getIndex()] = toUndo.getValue();
                toUndo = temp;
            }
        }
	}
